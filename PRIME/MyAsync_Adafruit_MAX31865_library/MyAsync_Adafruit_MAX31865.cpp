/**********
Author: Eric Broyles
Expansion onto Adafruit Max31865 code to allow for "async" calls to readRTD
Problem: code provided by adafruit has a hard coded delay(10) and delay(65) in readRTD, 
         this is fine for reading one sensor but if you have multiple you will be forced to wait increasing time.
Fix: split readRTD into 3 parts then when you read from the sensor call each part and handle the waits outside this.
**********/
...
/**************************************************************************/
//
MyAsync_Adafruit_MAX31865::MyAsync_Adafruit_MAX31865(int8_t spi_cs, int8_t spi_mosi,
...
*/
/**************************************************************************/
MyAsync_Adafruit_MAX31865::MyAsync_Adafruit_MAX31865(int8_t spi_cs, SPIClass *theSPI)
...
*/
/**************************************************************************/
bool MyAsync_Adafruit_MAX31865::begin(max31865_numwires_t wires) {
  spi_dev.begin();
...
*/
/**************************************************************************/
uint8_t MyAsync_Adafruit_MAX31865::readFault(max31865_fault_cycle_t fault_cycle) {
  if (fault_cycle) {
...
*/
/**************************************************************************/
void MyAsync_Adafruit_MAX31865::clearFault(void) {
...
*/
/**************************************************************************/
void MyAsync_Adafruit_MAX31865::enableBias(bool b) {
  uint8_t t = readRegister8(MAX31865_CONFIG_REG);
  if (b) {
...
*/
/**************************************************************************/
void MyAsync_Adafruit_MAX31865::autoConvert(bool b) {
  uint8_t t = readRegister8(MAX31865_CONFIG_REG);
  if (b) {
...
void MyAsync_Adafruit_MAX31865::enable50Hz(bool b) {
  uint8_t t = readRegister8(MAX31865_CONFIG_REG);
  if (b) {
...
*/
/**************************************************************************/
void MyAsync_Adafruit_MAX31865::setThresholds(uint16_t lower, uint16_t upper) {
...
*/
/**************************************************************************/
uint16_t MyAsync_Adafruit_MAX31865::getLowerThreshold(void) {
...
*/
/**************************************************************************/
uint16_t MyAsync_Adafruit_MAX31865::getUpperThreshold(void) {
...
*/
/**************************************************************************/
void MyAsync_Adafruit_MAX31865::setWires(max31865_numwires_t wires) {
  uint8_t t = readRegister8(MAX31865_CONFIG_REG);
...
*/
/**************************************************************************/
float MyAsync_Adafruit_MAX31865::temperature(float RTDnominal, float refResistor) {
...
*/
/**************************************************************************/
float MyAsync_Adafruit_MAX31865::calculateTemperature(uint16_t RTDraw, float RTDnominal,
                                              float refResistor) {
...
*/
/**************************************************************************/
uint16_t MyAsync_Adafruit_MAX31865::readRTD(void) {
  clearFault();
  enableBias(true);
  delay(10);
  uint8_t t = readRegister8(MAX31865_CONFIG_REG);
...
/**************************************************************************/
/*!
    @brief NEW split readRTD into 3 parts to allow async reading
*/
/**************************************************************************/
void MyAsync_Adafruit_MAX31865::readRTDPart1(void) {
  clearFault();
  enableBias(true);
  // you should delay for 10ms after this
}

void MyAsync_Adafruit_MAX31865::readRTDPart2(void) {
  uint8_t t = readRegister8(MAX31865_CONFIG_REG);
  t |= MAX31865_CONFIG_1SHOT;
  writeRegister8(MAX31865_CONFIG_REG, t);
  // you should delay for 65ms after this
}

uint16_t MyAsync_Adafruit_MAX31865::readRTDPart3(void) {
  uint16_t rtd = readRegister16(MAX31865_RTDMSB_REG);
  enableBias(false); // Disable bias current again to reduce selfheating.
  rtd >>= 1; // remove fault
  return rtd;
  //not the temp, must convert using calcTemp
}

/**********************************************/

uint8_t MyAsync_Adafruit_MAX31865::readRegister8(uint8_t addr) {
  uint8_t ret = 0;
...
uint16_t MyAsync_Adafruit_MAX31865::readRegister16(uint8_t addr) {
...
void MyAsync_Adafruit_MAX31865::readRegisterN(uint8_t addr, uint8_t buffer[],
                                      uint8_t n) {
...
void MyAsync_Adafruit_MAX31865::writeRegister8(uint8_t addr, uint8_t data) {
